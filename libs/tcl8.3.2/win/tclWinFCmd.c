...
static int		ConvertFileNameFormat(Tcl_Interp *interp, 
			    ...
			    Tcl_Obj **attributePtrPtr);
static int		DoCopyFile(const TCHAR *nativeSrc, const TCHAR *nativeDst);
...
static int		DoRenameFile(const TCHAR *nativeSrc, const TCHAR *nativeDst);
				 ...
				 * (UTF-8). */
{
    ...
    nativeDst = Tcl_WinUtfToTChar(dst, -1, &dstString);

    result = DoRenameFile(nativeSrc, nativeDst);
