...
class object {
  public:
	...
	const maybe_object<object> operator()(std::string const & idx) const {
		...
		Tcl_Obj *o = Tcl_GetVar2Ex(interp_, const_cast<char*>(name), const_cast<char*>(idx.c_str()), TCL_LEAVE_ERR_MSG);
		return maybe_object<object>(o, interp_, std::string(name), idx);
	}

	const bool exists(std::string idx) const {
		Tcl_Obj *array = obj_;
		Tcl_Obj *o = Tcl_GetVar2Ex(interp_, Tcl_GetString(array), const_cast<char*>(idx.c_str()), 0);
		return (o != 0);
	}
