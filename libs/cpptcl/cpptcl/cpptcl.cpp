...
#include <tclInt.h> // Tcl_CreateNamespace(...)
...
{
	auto obj = object(Tcl_GetObjResult(interp_));
	obj.set_interp(interp_);
	return obj;
}
...
void details::set_result(Tcl_Interp *interp, PyObject *p) {
	pybind11::object pyObj = pybind11::reinterpret_borrow<pybind11::object>(p);

	Tcl_Obj* tclObj = pythonToTclObject(pyObj);

	Tcl_SetObjResult(interp, tclObj);
}

void details::set_result(Tcl_Interp *interp, pybind11::object pyObj) {
	Tcl_Obj* tclObj = pythonToTclObject(pyObj);
...
object::object(char const *buf, size_t size) : interp_(0) {
	obj_ = Tcl_NewByteArrayObj(reinterpret_cast<unsigned char*>(const_cast<char *>(buf)), static_cast<int>(size));
	Tcl_IncrRefCount(obj_);
}
...
object &object::assign(char const *buf, size_t size) {
	Tcl_SetByteArrayObj(obj_, reinterpret_cast<unsigned char*>(const_cast<char *>(buf)), static_cast<int>(size));
	return *this;
}
...
object &object::assign(char const *s) {
	Tcl_SetStringObj(obj_, const_cast<char*>(s), -1);
	return *this;
}

object &object::assign(string const &s) {
	Tcl_SetStringObj(obj_, const_cast<char*>(s.data()), static_cast<int>(s.size()));
	return *this;
}
...
interpreter::interpreter(Tcl_Interp *interp, bool owner) {
	...
	owner_ = owner;
	if (!defaultInterpreter) {
		if (Tcl_InitStubs(interp, const_cast<char*>("8.3"), 0) == NULL) {
...
interpreter::interpreter(interpreter &&other) : interp_(other.interp_)
{
	owner_ = other.owner_ ;
	other.owner_ = false;
}
...
result interpreter::eval(string const &script) {
	int cc = Tcl_EvalEx(interp_, const_cast<char*>(script.c_str()), -1, 0);
	if (cc != TCL_OK) {
		throw tcl_error(interp_);
	}
...
result interpreter::eval(object const &o) {
	int cc = Tcl_EvalObjEx(interp_, o.get_object(), TCL_EVAL_DIRECT);
	if (cc != TCL_OK) {
		throw tcl_error(interp_);
	}
...
result interpreter::getVar(string const &variableName, int flags) {
	object n = object(variableName.c_str());
    Tcl_Obj * obj = Tcl_ObjGetVar2(interp_, n.get_object(), nullptr, flags);
	if (obj == NULL) {
		throw tcl_error(interp_);
	} else {
        Tcl_SetObjResult(interp_, obj);
...
result interpreter::setVar(string const &variableName, object const &scalarTclVariable, int flags) {
	Tcl_Obj * obj = Tcl_SetVar2Ex(interp_, const_cast<char*>(variableName.c_str()), 0, scalarTclVariable.get_object(), flags);
	if (obj == NULL) {
		throw tcl_error(interp_);
	} else {
        Tcl_SetObjResult(interp_, obj);
...
void interpreter::pkg_provide(string const &name, string const &version) {
	int cc = Tcl_PkgProvide(interp_, const_cast<char*>(name.c_str()), const_cast<char*>(version.c_str()));
	if (cc != TCL_OK) {
		throw tcl_error(interp_);
	}
}
...
void interpreter::create_namespace(string const &name) {
	if ( Tcl_CreateNamespace(interp_, const_cast<char*>(name.c_str()), nullptr, nullptr) == 0) {
		throw tcl_error(interp_);
	}
}
...
void interpreter::create_alias(string const &cmd, interpreter &targetInterp, string const &targetCmd) {
	int cc = Tcl_CreateAlias(interp_, const_cast<char*>(cmd.c_str()), targetInterp.interp_, const_cast<char*>(targetCmd.c_str()), 0, 0);
	if (cc != TCL_OK) {
		throw tcl_error(interp_);
	}
}
...
void interpreter::clear_definitions(Tcl_Interp *interp) {
...

	{
		...
		for (callback_interp_map::iterator it2 = imap.begin(); it2 != imap.end(); ++it2) {
			Tcl_DeleteCommand(interp, const_cast<char*>(it2->first.c_str()));
		}
...
void interpreter::add_function(string const &name, shared_ptr<callback_base> cb, policies const &p) {
	Tcl_CreateObjCommand(interp_, const_cast<char*>(name.c_str()), callback_handler, 0, 0);
...
void interpreter::add_constructor(string const &name, shared_ptr<class_handler_base> chb, shared_ptr<callback_base> cb, policies const &p) {
	Tcl_CreateObjCommand(interp_, const_cast<char*>(name.c_str()), constructor_handler, static_cast<ClientData>(chb.get()), 0);
